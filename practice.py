...
while True:
    canvasGUI()
    actual = int(input("What is the number you just drew: "))

    img= cv2.imread("images/roi_image.png")[:,:,0]
    # img= np.invert(np.array([img]))

    prediction = model.predict(img)

    print(f"I predict this digit is probably a {np.argmax(prediction)}")
    print (type((actual)))
    print (prediction)
    p = int(np.argmax(prediction))

    if (actual == p):
        print("I was right!")
    
    q = input("do you want to quit, enter anything to exit")
    if (q != ""):
        break